...
void Spw_Tx_init(Spw_Tx *const tx) { (void)tx; }
...
void Spw_Tx_setNextSendList(Spw_Tx *const tx,
			    const Spw_Tx_SendListConfig *const config)
{
	(void)tx; // Suppress unused parameter warning

	SPW_PKTTX_SetNextSendList(
		(uint8_t *)config->routerByte,
...
void Spw_Tx_getCurrentSendListConfig(const Spw_Tx *const tx,
				     Spw_Tx_SendListConfig *const config)
{
	(void)tx; // Suppress unused parameter warning
	...
	config->sendListAddress =
		(Spw_Tx_SendListEntry *)SPW_REGS->SPW_PKTTX1_CURSENDADDR;
	config->sendListLength =
		(uint16_t)((curSendCfg & SPW_PKTTX1_CURSENDCFG_LEN_Msk) >>
			   SPW_PKTTX1_CURSENDCFG_LEN_Pos);
	config->routerByte[0] =
		(uint8_t)((curSendRout & SPW_PKTTX1_NXTSENDROUT_BYTE1_Msk) >>
			  SPW_PKTTX1_NXTSENDROUT_BYTE1_Pos);
	config->routerByte[1] =
		(uint8_t)((curSendRout & SPW_PKTTX1_NXTSENDROUT_BYTE2_Msk) >>
			  SPW_PKTTX1_NXTSENDROUT_BYTE2_Pos);
	config->routerByte[2] =
		(uint8_t)((curSendRout & SPW_PKTTX1_NXTSENDROUT_BYTE3_Msk) >>
			  SPW_PKTTX1_NXTSENDROUT_BYTE3_Pos);
	config->routerByte[3] =
		(uint8_t)((curSendRout & SPW_PKTTX1_NXTSENDROUT_BYTE4_Msk) >>
...
uint32_t Spw_Tx_getAndClearIrqMasked(const Spw_Tx *const tx)
{
	...
	return SPW_PKTTX_IrqStatusGetMaskedAndClear() &
	       SPW_PKTTX1_IRQREGS_ALLIRQ_MASK;
}
...
void Spw_Tx_getStructuredIrq(const uint32_t linkIrqStatusValue,
			     Spw_Tx_IrqStatus *const status)
{
	status->eepSentIrqOccurred =
		(linkIrqStatusValue & (uint32_t)SPW_PKTTX_INT_MASK_EEP) != 0U;
	status->eopSentIrqOccurred =
		(linkIrqStatusValue & (uint32_t)SPW_PKTTX_INT_MASK_EOP) != 0U;
...
void Spw_Tx_getStatus(Spw_Tx *const tx, Spw_Tx_Status *const status)
{
	...
	status->lastState =
		(Spw_Tx_DeactivatedSendListState)SPW_PKTTX_STATUS_PREV_GET(
			rawStatus);
	status->isReadyToDeactivateCurrentSendList =
		(rawStatus & SPW_PKTTX_STATUS_DEACT) != 0U;
	status->isNextSendListPending =
		(rawStatus & SPW_PKTTX_STATUS_PENDING) != 0U;
	status->isCurrentSendListActive =
		(rawStatus & SPW_PKTTX_STATUS_ACT) != 0U;
