			    ...
			    const Spw_Tx_SendListConfig *const config)
{
	...
	SPW_PKTTX_SetNextSendList(
		(uint8_t *)config->routerByte, config->sendListAddress,
		config->sendListLength, config->abortOngoingSendListWhenStarted,
...
{
	status->eepSentIrqOccurred = (linkIrqStatusValue &
				      (uint32_t)SPW_PKTTX_INT_MASK_EEP) != 0U;
	status->eopSentIrqOccurred = (linkIrqStatusValue &
				      (uint32_t)SPW_PKTTX_INT_MASK_EOP) != 0U;
...
void Spw_Tx_getStatus(Spw_Tx *const tx, Spw_Tx_Status *const status)
{
	...
	status->lastState = (Spw_Tx_DeactivatedSendListState)
		SPW_PKTTX_STATUS_PREV_GET(rawStatus);
	status->isReadyToDeactivateCurrentSendList = (rawStatus &
						      SPW_PKTTX_STATUS_DEACT) !=
						     0U;
	status->isNextSendListPending = (rawStatus & SPW_PKTTX_STATUS_PENDING) !=
					0U;
	status->isCurrentSendListActive = (rawStatus & SPW_PKTTX_STATUS_ACT) !=
					  0U;
