...
void Spw_Rx_setConfig(Spw_Rx* const rx, const Spw_Rx_Config* const config)
{
  ...
  SPW_PKTRX_SetDiscard(config->overrunningPacketPolicy == Spw_Rx_OverrunningPacketPolicy_Discard);
...
uint32_t Spw_Rx_getAndClearIrqMasked(Spw_Rx* const rx)
{
  ...
  return SPW_PKTRX_IrqStatusGetMaskedAndClear() & SPW_PKTRX1_IRQREGS_ALLIRQ_MASK;
}
...
void Spw_Rx_setNextRxBuffer(Spw_Rx* const rx, const Spw_Rx_RxBufferConfig* const config)
{
  ...
  SPW_PKTRX_SetNextBuffer(config->rxDataAddress,
    config->rxDataLength,
...
void Spw_Rx_getNextRxBufferConfig(const Spw_Rx* const rx, Spw_Rx_RxBufferConfig* const config)
{
  ...
  config->rxBufferLength
    = (uint16_t)((nxtBufCfg & SPW_PKTRX1_NXTBUFCFG_MAXCNT_Msk) >> SPW_PKTRX1_NXTBUFCFG_MAXCNT_Pos);
  config->startValue
    = (uint8_t)((nxtBufCfg & SPW_PKTRX1_NXTBUFCFG_VALUE_Msk) >> SPW_PKTRX1_NXTBUFCFG_VALUE_Pos);
  config->startCondition = (Spw_Rx_StartCondition)((nxtBufCfg & SPW_PKTRX1_NXTBUFCFG_START_Msk)
                                                   >> SPW_PKTRX1_NXTBUFCFG_START_Pos);
  config->isPacketSplitAndDeactivationEnabled = (nxtBufCfg & SPW_PKTRX1_NXTBUFCFG_SPLIT_Msk) != 0U;
}
...
void Spw_Rx_getStatus(const Spw_Rx* const rx, Spw_Rx_Status* const status)
{
  ...
  status->numOfPacketsInActiveBuffer
    = (uint16_t)((rawStatus & SPW_PKTRX_STATUS_COUNT_MASK) >> SPW_PKTRX1_STATUS_COUNT_Pos);
  status->isIncomingPacketOngoing = (rawStatus & SPW_PKTRX_STATUS_PACKET) != 0U;
  status->isPreviousReceiveBufferLocked = (rawStatus & SPW_PKTRX_STATUS_LOCKED) != 0U;
  status->isNextReceiveBufferArmed = (rawStatus & SPW_PKTRX_STATUS_ARM) != 0U;
  status->isCurrentReceiveBufferActive = (rawStatus & SPW_PKTRX_STATUS_ACT) != 0U;
  status->isNextReceiveBufferPending = (rawStatus & SPW_PKTRX_STATUS_PENDING) != 0U;
  status->isReceiveBufferReadyToDeactivate = (rawStatus & SPW_PKTRX_STATUS_DEACT) != 0U;
}
  ...
  Spw_Rx_PreviousRxBufferStatus* const status)
{
  ...
  status->isBufferInfoFull = (rawStatus & SPW_PKTRX_PREV_STATUS_FULLI) != 0U;
  status->isBufferDataFull = (rawStatus & SPW_PKTRX_PREV_STATUS_FULLD) != 0U;
  status->hasDmaErrorOccurred = (rawStatus & SPW_PKTRX_PREV_STATUS_DMAERR) != 0U;
