...
void Spw_Link_setConfig(const Spw_Link* const link, const Spw_Link_Config* const config)
{
  ...
  SPW_LINK_EscapeCharEvent1Set(
    link->id, config->escEvent1.active, config->escEvent1.mask, config->escEvent1.value);
  SPW_LINK_EscapeCharEvent2Set(
    link->id, config->escEvent2.active, config->escEvent2.mask, config->escEvent2.value);
...
void Spw_Link_getConfig(const Spw_Link* const link, Spw_Link_Config* const config)
{
  ...
  config->txInitDiv
    = (uint8_t)((clkdiv & SPW_LINK1_CLKDIV_TXINITDIV_Msk) >> SPW_LINK1_CLKDIV_TXINITDIV_Pos);
  config->txOperDiv
    = (uint8_t)((clkdiv & SPW_LINK1_CLKDIV_TXOPERDIV_Msk) >> SPW_LINK1_CLKDIV_TXOPERDIV_Pos);
  config->command = (uint8_t)((cfg & SPW_LINK1_CFG_COMMAND_Msk) >> SPW_LINK1_CFG_COMMAND_Pos);

  config->escEvent1.active
    = (SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_ACTIVE_Msk) != 0U;
  config->escEvent1.mask
    = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_MASK_Msk)
                >> SPW_LINK1_ESCCHAREVENT0_MASK_Pos);
  config->escEvent1.value
    = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_VALUE_Msk)
                >> SPW_LINK1_ESCCHAREVENT0_VALUE_Pos);
  config->escEvent2.active
    = (SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_ACTIVE_Msk) != 0U;
  config->escEvent2.mask
    = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_MASK_Msk)
                >> SPW_LINK1_ESCCHAREVENT1_MASK_Pos);
  config->escEvent2.value
    = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_VALUE_Msk)
                >> SPW_LINK1_ESCCHAREVENT1_VALUE_Pos);

  config->escChar = (uint8_t)((SPW_REGS->SPW_LINK1_TRANSESC & SPW_LINK1_TRANSESC_CHAR_Msk)
                              >> SPW_LINK1_TRANSESC_CHAR_Pos);

  config->distributedIrqToEnable = 0;     // Set to 0, used only in setter
  config->distributedIrqToDisable = 0;    // Set to 0, used only in setter
  config->distributedAckIrqToEnable = 0;  // Set to 0, used only in setter
  config->distributedAckIrqToDisable = 0; // Set to 0, used only in setter
}
...
void Spw_Link_getStatus(const Spw_Link* const link, Spw_Link_Status* const status)
{
  ...
  status->wasConnectingStateSeen = (rawStatus & SPW_LINK_STATUS_SEEN4) != 0U;
  ...
  status->wasErrorResetStateSeen = (rawStatus & SPW_LINK_STATUS_SEEN0) != 0U;
  status->hasReceiverGotNchars = (rawStatus & SPW_LINK_STATUS_GOTNCHAR) != 0U;
  ...
  status->strappedResetValueForTxBitrates
    = (uint8_t)((rawStatus & SPW_LINK_STATUS_TXDEFDIV_MASK) >> SPW_LINK1_STATUS_TXDEFDIV_Pos);
  status->linkState = (Spw_Link_State)((rawStatus & SPW_LINK_STATUS_LINKSTATE_MASK)
                                       >> SPW_LINK1_STATUS_LINKSTATE_Pos);
  status->lastMatchingEscCharEvent1 = SPW_LINK_LastRecvEscapeCharEvent1Get(link->id);
  status->lastMatchingEscCharEvent2 = SPW_LINK_LastRecvEscapeCharEvent2Get(link->id);
}

void Spw_Link_setIrqMask(Spw_Link* const link, const uint32_t enable, const uint32_t disable)
{
  if (enable != 0U)
  {
...
void Spw_Link_getStructuredIrq(const uint32_t irq, Spw_Link_IrqStatus* const status)
{
...
uint32_t Spw_Link_getAndClearDistributedInterruptIrqMasked(const Spw_Link* const link)
{
  return SPW_LINK_DistIrqStatusGetMaskedAndClear(link->id);
}

void Spw_Link_clearDistributedInterruptIrq(Spw_Link* const link, const uint32_t mask)
{
...
uint32_t Spw_Link_getAndClearDistributedInterruptAckIrqMasked(const Spw_Link* const link)
{
  return SPW_LINK_DistAckIrqStatusGetMaskedAndClear(link->id);
}

void Spw_Link_clearDistributedInterruptAckIrq(Spw_Link* const link, const uint32_t mask)
{
  writeDistAckIrqClear(link->id, (uint32_t)mask);
}

void Spw_Link_setEscapeCharEvent1(Spw_Link* const link, const Spw_Link_EscCharEvent* const event)
{
  SPW_LINK_EscapeCharEvent1Set(link->id, event->active, event->mask, event->value);
}

void Spw_Link_setEscapeCharEvent2(Spw_Link* const link, const Spw_Link_EscCharEvent* const event)
{
