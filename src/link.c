...
void Spw_Link_setConfig(const Spw_Link_t *link, const Spw_Link_Config_t *config)
{
    ...
    {
        SPW_REGS->SPW_LINK2_CLKDIV = SPW_LINK2_CLKDIV_TXINITDIV(config->txInitDiv)
    ...
    SPW_LINK_EscapeCharEvent1Set(link->id, config->escEvent1.active, config->escEvent1.mask, config->escEvent1.value);
    SPW_LINK_EscapeCharEvent2Set(link->id, config->escEvent2.active, config->escEvent2.mask, config->escEvent2.value);
    ...
    SPW_LINK_DistAckInterruptEnable(link->id, config->distributedAckIrqToEnable);
    SPW_LINK_DistAckInterruptDisable(link->id, config->distributedAckIrqToDisable);
}
...
void Spw_Link_getConfig(const Spw_Link_t *link, Spw_Link_Config_t *config)
{
    ...
    config->txInitDiv = (uint8_t)((clkdiv & SPW_LINK1_CLKDIV_TXINITDIV_Msk) >> SPW_LINK1_CLKDIV_TXINITDIV_Pos);
    config->txOperDiv = (uint8_t)((clkdiv & SPW_LINK1_CLKDIV_TXOPERDIV_Msk) >> SPW_LINK1_CLKDIV_TXOPERDIV_Pos);
    config->command   = (uint8_t)((cfg    & SPW_LINK1_CFG_COMMAND_Msk)       >> SPW_LINK1_CFG_COMMAND_Pos);

    config->escEvent1.active = (SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_ACTIVE_Msk) != 0U;
    config->escEvent1.mask   = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_MASK_Msk) >> SPW_LINK1_ESCCHAREVENT0_MASK_Pos);
    config->escEvent1.value  = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT0 & SPW_LINK1_ESCCHAREVENT0_VALUE_Msk) >> SPW_LINK1_ESCCHAREVENT0_VALUE_Pos);
    config->escEvent2.active = (SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_ACTIVE_Msk) != 0U;
    config->escEvent2.mask   = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_MASK_Msk) >> SPW_LINK1_ESCCHAREVENT1_MASK_Pos);
    config->escEvent2.value  = (uint8_t)((SPW_REGS->SPW_LINK1_ESCCHAREVENT1 & SPW_LINK1_ESCCHAREVENT1_VALUE_Msk) >> SPW_LINK1_ESCCHAREVENT1_VALUE_Pos);

    config->escChar = (uint8_t)((SPW_REGS->SPW_LINK1_TRANSESC & SPW_LINK1_TRANSESC_CHAR_Msk) >> SPW_LINK1_TRANSESC_CHAR_Pos);

    config->distributedIrqToEnable = 0; // No way to read currently enabled interrupts
    config->distributedIrqToDisable = 0; // No way to read currently enabled interrupts
    config->distributedAckIrqToEnable = 0; // No way to read currently enabled interrupts
    config->distributedAckIrqToDisable = 0; // No way to read currently enabled interrupts
}
...
Spw_Link_Status_t Spw_Link_getStatus(const Spw_Link_t *link)
{
    ...
    decodedStatus.wasConnectingStateSeen = (status & SPW_LINK_STATUS_SEEN4) != 0U;
    decodedStatus.wasStartedStateSeen = (status & SPW_LINK_STATUS_SEEN3) != 0U;
    decodedStatus.wasReadyStateSeen = (status & SPW_LINK_STATUS_SEEN2) != 0U;
    decodedStatus.wasErrorWaitStateSeen = (status & SPW_LINK_STATUS_SEEN1) != 0U;
    decodedStatus.wasErrorResetStateSeen = (status & SPW_LINK_STATUS_SEEN0) != 0U;
    decodedStatus.hasReceiverGotNchars = (status & SPW_LINK_STATUS_GOTNCHAR) != 0U;
    decodedStatus.hasReceiverGotFct = (status & SPW_LINK_STATUS_GOTFCT) != 0U;
    decodedStatus.hasReceiverGotNull = (status & SPW_LINK_STATUS_GOTNULL) != 0U;
    decodedStatus.isTxBufferEmpty = (status & SPW_LINK_STATUS_TXEMPTY) != 0U;
    decodedStatus.strappedResetValueForTxBitrates = (uint8_t)((status & SPW_LINK_STATUS_TXDEFDIV_MASK) >> SPW_LINK1_STATUS_TXDEFDIV_Pos);
    decodedStatus.linkState = (Spw_Link_State_t)((status & SPW_LINK_STATUS_LINKSTATE_MASK) >> SPW_LINK1_STATUS_LINKSTATE_Pos);
    decodedStatus.lastMatchingEscCharEvent1 = SPW_LINK_LastRecvEscapeCharEvent1Get(link->id);
    decodedStatus.lastMatchingEscCharEvent2 = SPW_LINK_LastRecvEscapeCharEvent2Get(link->id);
...
{
    SPW_LINK_EscapeCharEvent1Set(link->id, event->active, event->mask, event->value);
}

void Spw_Link_setEscapeCharEvent2(const Spw_Link_t *link, const Spw_Link_EscCharEvent_t *event)
{
    SPW_LINK_EscapeCharEvent2Set(link->id, event->active, event->mask, event->value);
}
